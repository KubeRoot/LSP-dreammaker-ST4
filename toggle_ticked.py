...
from .plugin import DreamMakerST4
...
class DreammakerToggleTickedCommand(LspTextCommand):
	session_name = DreamMakerST4.name()

	def is_visible(self):
		return bool(is_tickable(self.view.file_name()))

	def run(self, _):
		session = self.session_by_name(DreamMakerST4.name())
		if session and session._plugin and isinstance(session._plugin,DreamMakerST4) and env_toggle_ticked(self.view.window(), self.view.file_name(), session._plugin):
			update_ticked_status(self.view, session._plugin)
		else:
...
def update_ticked_status(view, session:DreamMakerST4):
	fname = view.file_name()
	if not fname:
		view.erase_status(STATUS_KEY)
		return

	discovered = environment_path(view.window(), fname, session)
	if not discovered:
		view.erase_status(STATUS_KEY)
		return
...
def env_toggle_ticked(window, file_uri, session:DreamMakerST4):
	if not file_uri:
		return

	discovered = environment_path(window, file_uri, session)
	if not discovered:
		return
...
def environment_path(window, of:str, session:DreamMakerST4):
	folders = window.folders()
	if not folders:
		return

	if not session or not session.environment_file:
		return

	root = folders[0]
	relative = os.path.relpath(of, root)

	dme = os.path.join(root, session.environment_file)
